...
import { ArrowRight, BookOpen, PenLine, Search, ShoppingBag, Sparkles, BadgeCheck } from "lucide-react";
...
export default function HomePage() {
  ...
  let topMangas: any[] = [];
  let novels: NovelRow[] = [];
  let originalNovels: any[] = [];
  let originalMangas: any[] = [];
  ...
  try {
    ...
    novels = (db.prepare(`
      SELECT n.id, n.slug, n.title, n.title_en, n.title_jp, n.alternative_titles, n.cover_url, n.cover_local_path,
             n.type, n.genres, COALESCE(cc.is_original, 0) as is_original, cc.curation_label,
             COALESCE(cc.is_featured, n.is_featured, 0) as admin_featured, COALESCE(cc.show_on_home, 0) as show_on_home
      FROM novels n
      ...
      LIMIT 12
    `).all() as any[]).filter((n: any) => !hasCjk(n.title)).slice(0, 6);

    // Originais BR — novels com is_original=1
    originalNovels = db.prepare(`
      SELECT n.id, n.slug, n.title, n.title_en, n.title_jp, n.alternative_titles, n.cover_url, n.cover_local_path,
             n.type, n.genres, COALESCE(cc.curation_label, '') as curation_label
      FROM novels n
      ...
      ORDER BY cc.curation_label DESC, n.views DESC
      LIMIT 6
    `).all() as any[];

    // Originais BR — mangás com is_original=1
    originalMangas = db.prepare(`
      SELECT m.id, m.slug, m.title, m.cover_url, m.cover_local_path,
             COALESCE(cc.curation_label, '') as curation_label,
             (SELECT COUNT(*) FROM manga_chapters ch WHERE ch.manga_id = m.id AND EXISTS (SELECT 1 FROM manga_pages p WHERE p.chapter_id = ch.id AND coalesce(p.image_url, p.local_path, '') <> '')) AS chapter_count
      FROM mangas m
      ...
      ORDER BY cc.curation_label DESC, m.updated_at DESC
      LIMIT 6
    `).all() as any[];
  ...
  const hasOriginal = originalNovels.length > 0 || originalMangas.length > 0;
      ...
      <section className="container mx-auto max-w-7xl px-4 pb-16 pt-16 md:pt-24">
        <div className="reveal-up mx-auto max-w-4xl space-y-6 text-center">
          ...
          <form action="/search" className="glass-panel relative mx-auto max-w-2xl rounded-2xl p-1.5">
            ...
            />
            <button type="submit" className="absolute right-3 top-1/2 h-10 -translate-y-1/2 cursor-pointer rounded-xl bg-primary px-5 text-sm font-bold text-primary-foreground shadow-sm transition-colors hover:bg-primary/90">Buscar</button>
          </form>
        ...
        <section className="container mx-auto max-w-7xl px-4 pb-10">
          <div className="mb-5 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <h2 className="font-heading text-2xl font-black">
                <span className="gradient-text">Originais BR</span>
              </h2>
              <span className="rounded-full border border-emerald-500/20 bg-emerald-500/8 px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-emerald-400">
                <BadgeCheck className="mr-1 inline h-2.5 w-2.5" />
                Publicado aqui
              </span>
            </div>
            <Button variant="ghost" asChild className="hover:text-primary"><Link href="/explore?original=1">Ver originais <ArrowRight className="ml-1 h-4 w-4" /></Link></Button>
          </div>

          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-6">
            {/* Novels originais */}
            {originalNovels.map((n: any) => (
                ...
                <Card className="neon-card h-full overflow-hidden">
                  <div className="relative aspect-[3/4] overflow-hidden bg-muted">
                    {getCover(n) ? (
                      ...
                      <div className="flex h-full w-full items-center justify-center bg-gradient-to-br from-emerald-500/25 via-primary/10 to-cyan-500/10 p-3 text-center text-xs text-muted-foreground">
                        <NovelTitle novel={n} />
                      </div>
                    )}
                    <div className="absolute inset-x-0 top-0 flex justify-between p-2">
                      {n.curation_label ? (
                        <CurationBadge label={n.curation_label} size="sm" />
                      ) : (
                        <OriginalBadge size="sm" />
                      )}
                    </div>
                  </div>
                  <CardContent className="space-y-1 p-2.5">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {safeJsonArray(n.genres).slice(0, 2).map((g: string) => (
              ...
              <Link key={m.id} href={`/manga/${m.slug}`} className="group block">
                <Card className="neon-card overflow-hidden">
                  <div className="relative aspect-[2/3] overflow-hidden bg-muted">
                    ...
                    <div className="absolute inset-x-0 top-0 flex justify-between p-2">
                      {m.curation_label ? (
                        <CurationBadge label={m.curation_label} size="sm" />
                      ) : (
                        <OriginalBadge size="sm" />
                      )}
                    </div>
          ...
          <div className="mt-6 rounded-2xl border border-emerald-500/20 bg-gradient-to-r from-emerald-500/5 via-primary/5 to-cyan-500/5 p-5 text-center md:p-6">
            ...
            <div className="mt-3 flex justify-center gap-3">
              ...
              <Button variant="outline" asChild className="rounded-full"><Link href="/how-to">Como funciona</Link></Button>
            </div>
          </div>
        </section>
      )}
