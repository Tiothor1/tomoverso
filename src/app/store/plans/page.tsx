    ...
    <main className="container mx-auto max-w-7xl px-4 py-10">
            ...
            return (
              <div
                ...
                className={`relative flex flex-col rounded-2xl border p-6 ${
                  plan.id === "pro-yearly"
                    ? "border-amber-500/40 bg-gradient-to-b from-amber-500/10 to-card shadow-lg shadow-amber-500/5"
                    : "border-border/60 bg-card"
                }`}
              >
                {plan.id === "pro-yearly" && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-amber-500 px-4 py-1 text-xs font-bold text-amber-950">
                    Melhor valor
                  </div>
                )}
                ...
                <div className="my-4">
                  ...
                  {plan.interval === "year" && (
                    <p className="mt-1 text-xs text-emerald-500">
                      ~{formatBRL(monthlyPrice)}/mês · Economia de {formatBRL(savings)}
                    </p>
                  )}
                </div>
      ...
      <section className="mx-auto mt-16 max-w-2xl rounded-2xl border border-border/50 bg-card/50 p-6 text-center">
        ...
        <p className="mt-2 text-muted-foreground">
          Acesse o painel para gerenciar sua assinatura, ver histórico de pagamentos ou cancelar.
        </p>
        <Link
          href="/dashboard/subscription"
          className="mt-4 inline-block rounded-xl border border-border/60 px-6 py-2 text-sm font-medium hover:bg-accent"
        >
          Ir para o painel
        </Link>
      </section>
