...
import { createPreapproval, hasMercadoPagoCredentials } from "@/lib/subscriptions";
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Cria preapproval recorrente no Mercado Pago
    ...
    db.prepare(`
      INSERT INTO user_subscriptions (id, user_id, plan_id, status, current_period_start, current_period_end, mp_preapproval_id)
      VALUES (?, ?, ?, 'pending', ?, ?, ?)
    `).run(subId, user.id, planId, now, endDate.toISOString(), preapproval.preapprovalId);

    return new Response(
      `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Redirecionando...</title></head><body><script>location.href=${JSON.stringify(preapproval.checkoutUrl)}</script><noscript><meta http-equiv="refresh" content="0;url=${preapproval.checkoutUrl}"></noscript><p>Redirecionando para o Mercado Pago...</p></body></html>`,
      {
        status: 200,
    ...
    console.error("[preapproval] error:", err?.message);
