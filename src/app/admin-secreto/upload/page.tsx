...
import { ArrowLeft, Upload, FileText, Check, AlertTriangle, Loader2 } from "lucide-react";
                ...
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-red-400/60">
                      {item.file_type?.toUpperCase()} · {(item.file_size / 1024).toFixed(0)}KB
                      {item.detected_chapters > 0 && ` · ${item.detected_chapters} capítulos detectados`}
                    </p>
                  </div>
                </div>
                <Badge variant="outline" className={`text-[10px] shrink-0 ${
                  item.status === 'completed' ? 'border-green-800/30 text-green-400' :
                  item.status === 'processing' ? 'border-blue-800/30 text-blue-400' :
                  item.status === 'error' ? 'border-red-800/30 text-red-400' :
                  'border-amber-800/30 text-amber-400'
                }`}>
                  {item.status === 'completed' ? 'Pronto' :
                   item.status === 'processing' ? 'Processando' :
                   item.status === 'error' ? 'Erro' : 'Análise'}
                </Badge>
              </div>
            ))}
          </div>
